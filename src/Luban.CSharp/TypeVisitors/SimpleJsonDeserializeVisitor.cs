...
public class SimpleJsonDeserializeVisitor : ITypeFuncVisitor<string, string, int, string>
{
    ...
    {
        return $"{{ if (!{json}.IsBoolean) {x} = false; else {x} = {json}; }}";
    }

    public string Accept(TByte type, string json, string x, int depth)
    {
        return $"{{ if (!{json}.IsNumber) {x} = 0; else {x} = {json}; }}";
    }
    ...
    {
        return $"{{ if (!{json}.IsNumber) {x} = 0; else {x} = ({type.Apply(DeclaringTypeNameVisitor.Ins)}){json}.AsInt; }}";
    }

    public string Accept(TString type, string json, string x, int depth)
    {
        return $"{{ if (!{json}.IsString) {x} = \"\"; {x} = {json}; }}";
    }
    ...
    public string Accept(TArray type, string json, string x, int depth)
    {
        ...
        return $"{{ var {tempJsonName} = {json}; if (!{tempJsonName}.IsArray) {x} = new {typeStr}; else {{ int {_n} = {tempJsonName}.Count; {x} = new {typeStr}; int {__index} = 0; foreach (JSONNode {__e} in {tempJsonName}.Children) {{ {type.ElementType.Apply(DeclaringTypeNameVisitor.Ins)} {__v}; {type.ElementType.Apply(this, __e, __v, depth + 1)} {x}[{__index}++] = {__v}; }} }} }}";
    }
    ...
    public string Accept(TSet type, string json, string x, int depth)
    {
        ...
        string __json = $"__json{depth}";
        string tempJsonName = __json;
        return $"{{ var {tempJsonName} = {json}; if (!{tempJsonName}.IsArray) {x} = new {type.Apply(DeclaringTypeNameVisitor.Ins)}(); else {x} = new {type.Apply(DeclaringTypeNameVisitor.Ins)}(); foreach (JSONNode {__e} in {tempJsonName}.Children) {{ {type.ElementType.Apply(DeclaringTypeNameVisitor.Ins)} {__v}; {type.ElementType.Apply(this, __e, __v, depth + 1)} {x}.Add({__v}); }} }}";
    }
